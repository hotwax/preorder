...
app.config.globalProperties.$filters = {
  ...
  groupFeatures(featureHierarchy: any) {
    ...
    if (featureHierarchy) {
      ...
      // const sortedFeaturesList = Object.keys(featuresList)
      // .sort()
      // .reduce(function (acc: any, key: any) { 
      //     acc[key] = featuresList[key];
      //     return acc;
      // }, {});
      // return sortedFeaturesList;
      let keys = Object.keys(featuresList);
      let sortedFeaturesList ={};
      keys.forEach(key => {
        sortedFeaturesList[key] = featuresList
      })
    }
  },
