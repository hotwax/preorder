...
export default defineComponent({
  ...
  mounted() {
    ...
    const ls = new SecureLS({ encodingType: 'aes' , isCompression: false , encryptionSecret: process.env.VUE_APP_SECURITY_KEY});
    console.log(JSON.stringify(this.$route.redirectedFrom?.query.token));
    if(this.$route.redirectedFrom?.query.token){
    // const token = JSON.stringify(this.$route.redirectedFrom?.query.token);
    // window.localStorage.setItem('token', token);
    const newToken = ls.get(this.$route.redirectedFrom?.query.token);
