...
export default defineComponent({
  ...
  mounted() {
    ...
    if(this.$route.redirectedFrom?.query.cipherText){
    console.log(this.$route.redirectedFrom?.query);
    const ciphertext = this.$route.redirectedFrom?.query["cipherText"]?.toString();
    console.log(typeof(ciphertext));
    const bytes  = CryptoJS.AES.decrypt(ciphertext, process.env.VUE_APP_SECURITY_KEY).toString(CryptoJS.enc.Utf8);
     const decryptedData = JSON.parse(bytes);                                  
    console.log(decryptedData);
    }
    //  const bytes  = CryptoJS.AES.decrypt(ciphertext, process.env.VUE_APP_SECURITY_KEY).toString();
    //  const decryptedData = JSON.parse(bytes);  
    // console.log(decryptedData);
    // if(this.$route.redirectedFrom?.query.token){
    
    // const token = this.$route.redirectedFrom?.query.token.toString();
    // window.localStorage.setItem('token', token);
    // const newToken = ls.get('token');
    // this.store.dispatch("user/setUserToken", newToken);
    // this.$router.push('/');
    // }
    // const newToken = ls.get('token');
    // this.store.dispatch("user/setUserToken", newToken);
