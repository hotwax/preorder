      ...
      <div v-else>
        <ion-item  v-bind:key="product.groupValue" v-for="product in products" lines="none" @click="() => router.push({ name: 'Product-details', params: { id: product.groupValue }})">
          ...
          <ion-label>
            ...
            <p>{{getProduct(product.groupValue)}}</p>
            <p v-if="$filters.getFeatures(getProduct(product.groupValue)., '1/COLOR/')">{{ $t("Colors") }} : {{ $filters.getFeatures(getProduct(product.groupValue).featureHierarchy, '1/COLOR/') }} </p>
