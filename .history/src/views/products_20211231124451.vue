      ...
      <div v-else>
        <ion-item  v-bind:key="product.groupValue" v-for="product in products" lines="none" @click="() => router.push({ name: 'Product-details', params: { id: product.groupValue }})">
          ...
          <ion-label>
            ...
            <p :v-for="attribute in $filters.groupFeatures(getProduct(product.groupValue).featureHierarchy)) ">{{$filters.groupFeatures(getProduct(product.groupValue).featureHierarchy)}}</p>
