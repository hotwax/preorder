      ...
      <div v-else>
        <ion-item  v-bind:key="product.groupValue" v-for="product in products" lines="none" @click="() => router.push({ name: 'Product-details', params: { id: product.groupValue }})">
          ...
          <ion-label>
            ...
            <div v-for="attribute in ($filters.groupFeatures(getProduct(product.groupValue).featureHierarchy))" :key="attribute" >
              <p v-for="item in attribute" :key="item">
                {{attribute }} :{{ attribute}}
              </p>
              <!-- {{ Object.keys(attribute)[index]}} :{{ attribute}} -->
            </div>
