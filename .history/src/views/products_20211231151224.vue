      ...
      <div v-else>
        <ion-item  v-bind:key="product.groupValue" v-for="product in products" lines="none" @click="() => router.push({ name: 'Product-details', params: { id: product.groupValue }})">
          ...
          <ion-label>
            ...
            <p v-for="(attribute, index) in ($filters.groupFeatures(getProduct(product.groupValue).featureHierarchy))" :key="attribute" style="  text-transform: capitalize;" >{{ index }}: {{ attribute }}</p>
          </ion-label>
